...
def test(
    ...
    for batch_idx, sdata in enumerate(source_dataloader):
        ...
        constant = 2.0 / (1.0 + np.exp(-10 * p)) - 1.0
        ...
        src_preds = domain_classifier(feature_extractor(input1))
        # src_preds = torch.argmax(src_preds, dim=1)
    ...
    for batch_idx, tdata in enumerate(target_dataloader):
        ...
        # tgt_preds  = torch.argmax(tgt_preds, dim=1)
    ...
    print(
        ...
        "Domain Accuracy: {}/{} ({:.4f}%)\n".format(
            ...
            100.0 * float(source_correct) / len(source_dataloader.dataset),
            target_correct,
            len(target_dataloader.dataset),
            100.0 * float(target_correct) / len(target_dataloader.dataset),
            domain_correct,
            len(source_dataloader.dataset) + len(target_dataloader.dataset),
            100.0
            * float(domain_correct)
            / (len(source_dataloader.dataset) + len(target_dataloader.dataset)),
        )
    )
